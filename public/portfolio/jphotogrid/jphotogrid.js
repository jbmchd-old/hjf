...
(function($) {
	$.fn.jphotogrid = function(settings, callback) {
		...
		function hideSelected(callback){
			...
			$('#'+ settings.id).filter('.' + settings.selectedClass).each(function(){
		...
		function select(el){
			hideSelected(function(){
				...
				$(el).stop(true).animate(settings.selectedCSS, 'slow');
			});
		}
		...
		return $(this).each(function(){
			...
			$('#'+ settings.id + '.' + settings.activeClass).live('click', function(){	
				select(this);
			});
			$('#'+ settings.id + '.' + settings.selectedClass).live('click', function(){
				hideSelected();
			});
